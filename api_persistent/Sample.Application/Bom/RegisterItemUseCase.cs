...
public sealed class RegisterItemUseCase(IBomRepository repository)
{
        ...
        RegisterItemCommand command, CancellationToken ct = default)
    {
        ...
        var outcome = await repository.RegisterAsync(item, ct);
        return outcome switch
        {
            WriteOutcome.Success => new RegisterItemResult.Success(),
            WriteOutcome.Conflict c => new RegisterItemResult.Conflict(c.Message),
            _ => throw new InvalidOperationException("未知のWriteOutcome")
        };
    }
}
